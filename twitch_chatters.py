...
# example: chat disabled, or chat hosten not on the twitch site
exceptions = ["destiny", "scglive"]
        ...
        for i in range(0, len(gamedata['streams'])):
            ...
            if (ratio < ratio_threshold):
                ...
                if not found:
                    ...
                    tweet = name + " (playing " + game.split(":")[0] + ") appears to have a false-viewer bot. (~" + str(viewers - chatters) + " extra viewers)"
                    if (len(tweet) + 2 + len(originame) <= 140):
